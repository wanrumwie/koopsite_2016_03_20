...
class FlatSchemeUsersPageAuthenticatedVisitorWithPermsTest(FlatSchemeUsersPageVisitTest):
    """
...
@skipIf(SKIP_TEST, "пропущено для економії часу")
class FlatSchemeUsersPageAuthenticatedVisitorWoPermsTest(FlatSchemeUsersPageVisitTest):
    """
    Тест відвідання сторінки сайту
    анонімним користувачем
    Параметри сторінки описані в суперкласі, тому не потребують переозначення.
...
class FlatSchemeUsersPageGoToFlatTest(FlatSchemeUsersPageVisitTest):
    ...
    def test_visitor_can_go_to_flat(self):
        ...
        url_name             = 'flats:flat-users-list'
        kwargs               = {'pk': flat.id}
        expected_regex       = ""
